...
class AuthService {
  async login(credentials: LoginCredentials): Promise<{ user: User; token: string }> {
    ...
    try {
      ...
      // Extract the actual user and token data from the nested structure
  ...
  async forgotPassword(forgotData: ForgotPasswordData): Promise<boolean> {
    ...
    try {
      ...
      // The backend returns: { success: boolean, data: {...} }
      // We need to return the success status from the response
